...
let editModeToggle: HTMLElement;
...
function initializeElements() {
    ...
    if (!controlPanelEl) throw new Error('Control panel element not found');
    controlPanel = controlPanelEl;
    
    const controlPanelBodyEl = controlPanel.querySelector('.control-panel-body');
    if (!controlPanelBodyEl) throw new Error('Control panel body not found');
    controlPanelBody = controlPanelBodyEl as HTMLElement;
    
    const minimizeBtnEl = document.getElementById('minimize-btn');
    if (!minimizeBtnEl) throw new Error('Minimize button not found');
    minimizeBtn = minimizeBtnEl;
    
    const editModeToggleEl = document.getElementById('edit-mode-toggle');
    if (!editModeToggleEl) throw new Error('Edit mode toggle not found');
    editModeToggle = editModeToggleEl;
    
    const layoutSelectEl = document.getElementById('layout-select') as HTMLSelectElement;
    if (!layoutSelectEl) throw new Error('Layout select not found');
    layoutSelect = layoutSelectEl;
    
    const themeSelectEl = document.getElementById('theme-select') as HTMLSelectElement;
    if (!themeSelectEl) throw new Error('Theme select not found');
    themeSelect = themeSelectEl;
    
    const saveBtnEl = document.getElementById('save-btn');
    if (!saveBtnEl) throw new Error('Save button not found');
    saveBtn = saveBtnEl;
    
    const resetBtnEl = document.getElementById('reset-btn');
    if (!resetBtnEl) throw new Error('Reset button not found');
    resetBtn = resetBtnEl;
    
    // Color related elements with null checks
    const colorBarEl = document.getElementById('color-bar') as HTMLInputElement;
    if (!colorBarEl) throw new Error('Color bar not found');
    colorBar = colorBarEl;
    
    const lightnessSliderEl = document.getElementById('lightness-slider') as HTMLInputElement;
    if (!lightnessSliderEl) throw new Error('Lightness slider not found');
    lightnessSlider = lightnessSliderEl;
    
    const saturationSliderEl = document.getElementById('saturation-slider') as HTMLInputElement;
    if (!saturationSliderEl) throw new Error('Saturation slider not found');
    saturationSlider = saturationSliderEl;
    
    const primaryColorPickerEl = document.getElementById('primary-color') as HTMLInputElement;
    if (!primaryColorPickerEl) throw new Error('Primary color picker not found');
    primaryColorPicker = primaryColorPickerEl;
    
    const secondaryColorPickerEl = document.getElementById('secondary-color') as HTMLInputElement;
    if (!secondaryColorPickerEl) throw new Error('Secondary color picker not found');
    secondaryColorPicker = secondaryColorPickerEl;
    
    const accentColorPickerEl = document.getElementById('accent-color') as HTMLInputElement;
    if (!accentColorPickerEl) throw new Error('Accent color picker not found');
    accentColorPicker = accentColorPickerEl;
    
    const colorIndicatorEl = document.getElementById('color-indicator');
    if (!colorIndicatorEl) throw new Error('Color indicator not found');
    colorIndicator = colorIndicatorEl;
    
    const colorPreviewBoxEl = document.getElementById('color-bar-preview');
    if (!colorPreviewBoxEl) throw new Error('Color preview box not found');
