...
class TableThemeComponent extends HTMLElement {
  ...
  applyTableThemeToDocument(extendedTheme) {
    ...
    root.style.setProperty('--table-compact-padding', `${tableTheme.compactPadding}px`);    // Apply enhanced table-specific color variables based on base theme
    if (baseColors) {
      console.log('🎨 Applying table color variables:', baseColors);
      root.style.setProperty('--table-header-bg', baseColors.primaryLight);
      root.style.setProperty('--table-header-color', baseColors.primaryDark);
      root.style.setProperty('--table-border-color', baseColors.border);
      root.style.setProperty('--table-stripe-bg', `${baseColors.primary}${Math.round(tableTheme.stripedOpacity * 255).toString(16).padStart(2, '0')}`);
      root.style.setProperty('--table-hover-color', `${baseColors.primary}${Math.round(tableTheme.hoverOpacity * 255).toString(16).padStart(2, '0')}`);
