...
const BaseClass = window.WBBaseComponent || HTMLElement;
    ...
    class WBResizePanel extends BaseClass {
        ...
        setupEventListeners() {
            const dragHandle = this.asHTMLElement(this.querySelector('#drag-handle'));
            const resizeHandle = this.asHTMLElement(this.querySelector('#resize-handle'));
        ...
        startDrag(e) {
            ...
            const targetEl = this.asHTMLElement(this.targetElement);
        ...
        onMouseMove(e) {
            const targetEl = this.asHTMLElement(this.targetElement);
            if (this.isDragging && targetEl) {
                const deltaX = e.clientX - this.startX;
                const deltaY = e.clientY - this.startY;
                targetEl.style.left = (this.initialX + deltaX) + 'px';
                targetEl.style.top = (this.initialY + deltaY) + 'px';
            } else if (this.isResizing && targetEl) {
                const deltaX = e.clientX - this.startX;
                const deltaY = e.clientY - this.startY;
        ...
        restoreState() {
            ...
            const targetEl = this.asHTMLElement(this.targetElement);
            if (!targetEl) return;
            if (savedX) targetEl.style.left = savedX + 'px';
            if (savedY) targetEl.style.top = savedY + 'px';
            if (savedWidth) targetEl.style.width = savedWidth + 'px';
            if (savedHeight) targetEl.style.height = savedHeight + 'px';
        }
    }
