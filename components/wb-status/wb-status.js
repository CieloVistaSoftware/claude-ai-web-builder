...
window.WBStatus = {
    ...
    getConfig: function() {
        const currentStatus = window._currentWBStatus;
        return currentStatus ? currentStatus.config : null;
    },
    
    // Create a status bar
    create: function(options = {}) {
        const statusBar = document.createElement('wb-status');
        
        // Set attributes from options
        if (options.position) statusBar.setAttribute('position', options.position);
        if (options.maxEvents) statusBar.setAttribute('max-events', options.maxEvents);
        if (options.eventDuration) statusBar.setAttribute('event-duration', options.eventDuration);
        if (options.fadeDelay) statusBar.setAttribute('fade-delay', options.fadeDelay);
        if (options.height) statusBar.setAttribute('height', options.height);
        
        return statusBar;
    },
    
    // Add event to current status bar
    addEvent: function(message, type = 'info', options = {}) {
        const currentStatus = window._currentWBStatus;
        if (currentStatus) {
            // Filter out color-related messages for smart grouping
            const colorKeywords = [
                'color', 'hue', 'saturation', 'lightness', 'primary', 'background',
                'hsl', 'hex', 'rgb', 'changed to:', 'palette'
            ];
            const isColorEvent = colorKeywords.some(keyword => 
                message.toLowerCase().includes(keyword.toLowerCase())
            );
            
            if (isColorEvent) {
                currentStatus.handleColorEvent(message, type);
            } else {
                currentStatus.addEvent(message, type, options);
            }
        }
    },
    
    // Update setting in current status bar
    updateSetting: function(key, value, isError = false) {
        const currentStatus = window._currentWBStatus;
        if (currentStatus) {
            currentStatus.updateSetting(key, value, isError);
        }
    },
    ...
    getCurrent: function() {
        return window._currentWBStatus;
    }
};

// Legacy compatibility - update global updateStatus function
window.updateStatus = function(message, type) {
    window.WBStatus.addEvent(message, type);
};
