...
window.WBCard = {
    create: function(content = {}, options = {}) {
        ...
        if (content.title) card.setAttribute('title', content.title);
        if (content.subtitle) card.setAttribute('subtitle', content.subtitle);
        if (content.body) card.setAttribute('body', content.body);
        if (content.footer) card.setAttribute('footer', content.footer);
        
        // Set media attributes
        if (content.media) {
            card.setAttribute('media-src', content.media.src);
            if (content.media.alt) card.setAttribute('media-alt', content.media.alt);
            if (content.media.type) card.setAttribute('media-type', content.media.type);
        }
        
        // Set actions
        if (content.actions && content.actions.length > 0) {
            card.setAttribute('actions', JSON.stringify(content.actions));
        ...
        if (options.layout) card.setAttribute('layout', options.layout);
        if (options.clickable) card.setAttribute('clickable', '');
        if (options.loading) card.setAttribute('loading', '');
        if (options.showHeader === false) card.setAttribute('show-header', 'false');
        if (options.showFooter) card.setAttribute('show-footer', 'true');
        if (options.showActions) card.setAttribute('show-actions', 'true');
        
        return card;
    },
    
    setContent: function(card, content) {
        if (card && card.setContent) {
            card.setContent(content);
        }
    },
    
    setVariant: function(card, variant) {
        if (card && card.setVariant) {
            card.setVariant(variant);
        }
    },
    
    setLayout: function(card, layout) {
        if (card && card.setLayout) {
            card.setLayout(layout);
        }
    },
    
    setLoading: function(card, loading) {
