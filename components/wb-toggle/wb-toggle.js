...
window.WBToggle = {
    create: function(options = {}) {
        const toggle = document.createElement('wb-toggle');
        
        // Set attributes from options
        if (options.id) toggle.setAttribute('id', options.id);
        if (options.label) toggle.setAttribute('label', options.label);
        if (options.checked) toggle.setAttribute('checked', '');
        if (options.disabled) toggle.setAttribute('disabled', '');
        if (options.size) toggle.setAttribute('size', options.size);
        if (options.labelPosition) toggle.setAttribute('label-position', options.labelPosition);
        if (options.variant) toggle.setAttribute('variant', options.variant);
        
        // Handle callback functions
        if (options.onchange || options.onfocus || options.onblur) {
            toggle.addEventListener('wbToggleChange', (e) => {
                if (options.onchange) options.onchange(e.detail.checked, toggle);
            });
            toggle.addEventListener('focus', (e) => {
                if (options.onfocus) options.onfocus(e, toggle);
            });
            toggle.addEventListener('blur', (e) => {
                if (options.onblur) options.onblur(e, toggle);
            });
        }
    ...
    setChecked: function(toggle, checked) {
        if (toggle && toggle.setChecked) {
            toggle.setChecked(checked);
        }
    },
    
    getChecked: function(toggle) {
        return toggle && toggle.getChecked ? toggle.getChecked() : false;
    },
    
    setDisabled: function(toggle, disabled) {
        if (toggle && toggle.setDisabled) {
            toggle.setDisabled(disabled);
        }
    },
    
    setLabel: function(toggle, label) {
        if (toggle && toggle.setLabel) {
