export {};
// @ts-nocheck
// Color Mapper for Website Builder
// This utility maps existing color variables to match our theme system

(function () {
  // Store original values to be able to restore them if needed
  let originalValues: any = {};
  let initialized = false;

  // Function to store original CSS variable values before they're overridden
  function storeOriginalValues(): any {
    // Get computed style
    const computedStyle = getComputedStyle(document.documentElement);

    // Store original values of common variables we'll be modifying
    ...
    variablesToStore.forEach(varName => {
      if (varName.startsWith('--')) {
        originalValues[varName] = computedStyle.getPropertyValue(varName);
      } else {
  ...
  function mapThemeColors(): any {
    ...
    // Define the CSS mapping rules for each theme
    const mappings = {      // Light theme (default)
      light: {
        // Main backgrounds
        ...
        // Elementor specific variables (from screenshot)
    ...
    // Apply the mapping for the current theme
    if (mappings[currentTheme]) {
      const style = document.createElement('style');
      style.id = 'wb-color-mapper';
      // First define variables on root elements
      let cssText = `:root[data-theme="${currentTheme}"],\n`;
      cssText += `body[data-theme="${currentTheme}"],\n`;
      cssText += `html[data-theme="${currentTheme}"] {\n`;

      for (const [siteVar, ourVar] of Object.entries(mappings[currentTheme])) {
      ...
      // Create more specific selectors to override other styles
      cssText += `html[data-theme="${currentTheme}"], body[data-theme="${currentTheme}"] {\n`;
      if (mappings[currentTheme]['color']) {
        cssText += `  color: ${mappings[currentTheme]['color']};\n`;
      }
      if (mappings[currentTheme]['background-color']) {
        cssText += `  background-color: ${mappings[currentTheme]['background-color']};\n`;
      }
      cssText += `}\n`;

      // Text elements
      cssText += `body[data-theme="${currentTheme}"] p,\n`;
      cssText += `body[data-theme="${currentTheme}"] li,\n`;
      cssText += `body[data-theme="${currentTheme}"] span:not([class*="elementor"]),\n`;
      cssText += `body[data-theme="${currentTheme}"] div:not([class*="elementor"]) {\n`;
      cssText += `  color: var(--text-color, ${mappings[currentTheme]['--text-color']});\n`;
      cssText += `}\n`;

      // Heading elements
      cssText += `body[data-theme="${currentTheme}"] h1, body[data-theme="${currentTheme}"] h2, `;
      cssText += `body[data-theme="${currentTheme}"] h3, body[data-theme="${currentTheme}"] h4, `;
      cssText += `body[data-theme="${currentTheme}"] h5, body[data-theme="${currentTheme}"] h6 {\n`;
      cssText += `  color: var(--heading-color, ${mappings[currentTheme]['--heading-color']});\n`;
      cssText += `}\n`;

      // Link elements
      cssText += `body[data-theme="${currentTheme}"] a:not([class*="elementor"]) {\n`;
      cssText += `  color: var(--link-color, ${mappings[currentTheme]['--link-color']});\n`;
      cssText += `}\n`;

      // Handle Elementor elements if found
      if (document.querySelector('.elementor') || document.querySelector('[class*="elementor"]')) {
        // Elementor root element
        cssText += `.elementor[data-theme="${currentTheme}"], .elementor-kit-5[data-theme="${currentTheme}"] {\n`;

        // Apply Elementor variables 
        for (const [siteVar, ourVar] of Object.entries(mappings[currentTheme])) {
        ...
        // High specificity overrides for Elementor text
        cssText += `.elementor[data-theme="${currentTheme}"] .elementor-text-editor,\n`;
        cssText += `.elementor[data-theme="${currentTheme}"] .elementor-widget-text-editor,\n`;
        cssText += `.elementor[data-theme="${currentTheme}"] p {\n`;
        cssText += `  color: var(--text-color, ${mappings[currentTheme]['--text-color']});\n`;
        cssText += `}\n`;

        // Elementor headings
        cssText += `.elementor[data-theme="${currentTheme}"] .elementor-heading-title,\n`;
        cssText += `.elementor[data-theme="${currentTheme}"] h1.elementor-heading-title,\n`;
        cssText += `.elementor[data-theme="${currentTheme}"] h2.elementor-heading-title,\n`;
        cssText += `.elementor[data-theme="${currentTheme}"] h3.elementor-heading-title {\n`;
        cssText += `  color: var(--heading-color, ${mappings[currentTheme]['--heading-color']});\n`;
        cssText += `}\n`;
      }
      ...
      // Remove any existing mapper styles
  ...
  function observeThemeChanges(): any {
    ...
    document.addEventListener('wb-theme-changed', (event): any => {
  ...
  function applyDirectCSSVariables(theme): any {
    if (!mappings[theme]) return;

    const themeVars = mappings[theme];

    // Apply to both documentElement and body for maximum compatibility
    [document.documentElement, document.body].forEach(element => {
      for (const [siteVar, ourVar] of Object.entries(themeVars)) {
        ...
        // If it's not a var reference or we couldn't resolve it, use the raw value
        element.style.setProperty(siteVar, ourVar);
      }

      // For non-CSS variable styles like color and background-color
      if (themeVars['color']) {
    ...
    // Handle Elementor-specific variables if Elementor is detected
    if (document.querySelector('.elementor') || document.querySelector('[class*="elementor"]')) {
      // Find and apply to Elementor elements
      ...
      elementorSelectors.forEach(selector => {
        ...
        elements.forEach(element => {
          ...
          // Also force additional properties for more reliable color changes
          ...
          if (selector === '.elementor-section') {
            if (themeVars['background-color']) {
              // Only set if not already styled
  ...
  function debugColorVariables(): any {
    const styles = getComputedStyle(document.documentElement);
    const allProps: any[] = [];

    // Check common color-related variable names
    const colorKeywords = ['color', 'bg', 'background', 'border', 'shadow', 'accent', 'primary', 'secondary', 'link', 'text', 'heading', 'global'];

    for (let i = 0; i < styles.length; i++) {
      const prop = styles[i];
      if (prop.startsWith('--') && colorKeywords.some(keyword => prop.toLowerCase().includes(keyword))) {
    ...
    allProps.forEach(({ prop, value }): any => {
  ...
  function debugCurrentThemeMappings(): any {
    const currentTheme = document.documentElement.getAttribute('data-theme') || 'light';
  ...
  function diagnoseColorIssues(): any {
    const currentTheme = document.documentElement.getAttribute('data-theme') || 'light';
    const results = {
    ...
    // Check for elements with potential color issues
    const allElements = document.querySelectorAll('*');
    const ignoreClasses = ['wb-controller', 'control-panel'];

    allElements.forEach(el => {
      // Skip our controller components
      ...
      // Check if element has inline style that might be blocking our theme
  ...
  window.debugWbColorVariables = debugColorVariables;
  window.debugWbThemeMappings = debugCurrentThemeMappings;
  window.diagnoseWbColorIssues = diagnoseColorIssues;
