...
(function() {
    ...
    class ControlPanel extends HTMLElement {
        ...
        handleThemeChange(e) {
            ...
            const themeChangeEvent = new CustomEvent('wb:theme-changed', {
                detail: { 
                    theme: theme,
                    source: 'control-panel',
                    timestamp: Date.now()
            ...
            // Log the change
            logEvent('info', `Theme changed to: ${theme} - Event dispatched`);
            console.log(`🎨 Fired theme change event: ${theme}`);
            console.log('📡 wb-theme component and all listeners will react to this event');
        }
        ...
        toggleMinimize() {
            ...
            console.log(`Control panel ${isMinimized ? 'restored' : 'minimized'}`);
        }
        ...
        logDebugInfo() {
            ...
            const debugInfo = {
                ...
                currentTheme: this.querySelector('#theme-select')?.value || 'unknown',
                currentLayout: this.querySelector('#layout-select')?.value || 'unknown',
                dependencies: {
        ...
        setInitialColorsOnComponents() {
            ...
            if (primaryColorBar) {
                if (savedPrimary) {
                    ...
                    primaryColorBar.setAttribute('text-hue', colorData.hue);
                    primaryColorBar.setAttribute('text-saturation', colorData.saturation);
                    primaryColorBar.setAttribute('text-lightness', colorData.lightness);
                } else {
                    // Use defaults from config
                    primaryColorBar.setAttribute('text-hue', this.currentColors.primary.hue);
                    primaryColorBar.setAttribute('text-saturation', this.currentColors.primary.saturation);
                    primaryColorBar.setAttribute('text-lightness', this.currentColors.primary.lightness);
                }
            }
            ...
            if (bgColorBar) {
                if (savedBg) {
                    ...
                    bgColorBar.setAttribute('text-hue', colorData.hue);
                    bgColorBar.setAttribute('text-saturation', colorData.saturation);
                    bgColorBar.setAttribute('text-lightness', colorData.lightness);
                } else {
                    // Use defaults from config
                    bgColorBar.setAttribute('text-hue', this.currentColors.background.hue);
                    bgColorBar.setAttribute('text-saturation', this.currentColors.background.saturation);
                    bgColorBar.setAttribute('text-lightness', this.currentColors.background.lightness);
                }
            }
        ...
        /**
         * Dynamically find navigation element without hardcoded selectors
         * @returns {Element|null} Navigation element
         */
        findNavigationElement() {
            // Try wb-nav custom element first
            let nav = document.querySelector('wb-nav');
            if (nav) return nav;
            
            // Look for WB navigation component
            nav = document.querySelector('[data-wb-component="navigation"]');
            if (nav) return nav;
            
            // Look for elements with navigation role
            nav = document.querySelector('[role="navigation"]');
            if (nav) return nav;
            
            // Try semantic nav element as fallback
            nav = document.querySelector('nav');
            if (nav) return nav;
            
            // Look for common navigation patterns (but not hardcoded classes)
            const navCandidates = document.querySelectorAll('header nav, .nav, [class*="nav-"], [id*="nav"]');
            if (navCandidates.length > 0) {
                // Return the first one that looks like a main navigation
                for (const candidate of navCandidates) {
                    if (candidate.querySelector('ul, ol') || candidate.querySelectorAll('a').length > 1) {
                        return candidate;
                    }
                }
            }
