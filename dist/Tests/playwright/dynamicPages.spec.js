 ...
 */
test_1.test.describe("Dynamic Pages Functionality Tests", () => {
    ...
    (0, test_1.test)("should have dynamic page creation functions in JavaScript", async ({ page, }) => {
        ...
        const dynamicPageFunctionsExist = await page.evaluate(() => {
            try {
                ...
                const hasSetupFunction = scripts.includes("function setupDynamicPagesNavigation()") ||
                    scripts.includes("setupDynamicPagesNavigation =");
                const hasCreatePageFunction = scripts.includes("function createNewPage(pageName)") ||
                    scripts.includes("createNewPage =");
                const hasShowPageFunction = scripts.includes("function showPage(pageName)") ||
                    scripts.includes("showPage =");
                const hasNavEventListener = scripts.includes("document.addEventListener('click'") ||
                ...
                return {
                    ...
                    hasShowPageFunction,
                    hasNavEventListener,
                    allFunctionsExist: hasSetupFunction &&
                        hasCreatePageFunction &&
                        hasShowPageFunction &&
                        hasNavEventListener,
                };
        ...
        console.log("Dynamic page function check results:", dynamicPageFunctionsExist);
        // Assert that all required functions exist
        if ("error" in dynamicPageFunctionsExist) {
            throw new Error(`Error checking dynamic page functions: ${dynamicPageFunctionsExist.error}`);
        }
        (0, test_1.expect)(dynamicPageFunctionsExist.allFunctionsExist).toBeTruthy();
    });
    (0, test_1.test)("should have main content container for pages", async ({ page }) => {
    ...
    (0, test_1.test)("should navigate to a new page when a link with hash is clicked", async ({ page, }) => {
        // Go to the base page
        await page.goto("/wb/wb/wb/wb.html");
        await page.waitForLoadState("networkidle");
        ...
        try {
            ...
            await (0, test_1.expect)(pageElement).toBeVisible({ timeout: 5000 });
            // Check if the page has the correct title
            const pageTitle = await page.locator("#page-testpage .site-title");
            await (0, test_1.expect)(pageTitle).toBeVisible({ timeout: 3000 });
            const titleText = await pageTitle.textContent();
            (0, test_1.expect)(titleText).not.toBeNull();
            (0, test_1.expect)(titleText).toContain("Testpage");
        }
        catch (error) {
    ...
    (0, test_1.test)("should navigate between pages when nav links are clicked", async ({ page, }) => {
        // Go to the base page
        await page.goto("/wb/wb/wb/wb.html");
        await page.waitForLoadState("networkidle");
        try {
            ...
            await (0, test_1.expect)(page1).toBeVisible({ timeout: 5000 });
            ...
            await (0, test_1.expect)(page2).toBeVisible({ timeout: 5000 });
            // First page should now be hidden
            const page1Hidden = await page.evaluate(() => {
                const page1 = document.querySelector("#page-testpage1");
                return !page1 || page1.style.display === "none";
            });
            (0, test_1.expect)(page1Hidden).toBeTruthy();
        }
        catch (error) {
            // Take screenshot if the test fails
...
//# sourceMappingURL=dynamicPages.spec.js.map