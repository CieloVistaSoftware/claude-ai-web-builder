...
(0, test_1.test)("Dynamic pages should be positioned within main-content", async ({ page, }) => {
    ...
    const parentOfDynamicPage = await page.$eval("#about", (el) => el.parentElement.id);
    // Check if the dynamic page is properly positioned within main-content
    (0, test_1.expect)(parentOfDynamicPage).toBe("main-content");
    // Check all dynamic pages to ensure they're in the main content
    const dynamicPagesCount = await page.$$eval(".dynamic-page", (elements) => elements.length);
    const dynamicPagesInMainContent = await page.$$eval("#main-content > .dynamic-page", (elements) => elements.length);
    // All dynamic pages should be direct children of main-content
    (0, test_1.expect)(dynamicPagesCount).toEqual(dynamicPagesInMainContent);
    ...
    const servicesSectionParent = await page.$eval("#services", (el) => el.parentElement.id);
    (0, test_1.expect)(servicesSectionParent).toBe("main-content");
});
// Test observer functionality
(0, test_1.test)("MutationObserver should move dynamically added sections to main-content", async ({ page, }) => {
    await page.goto("http://localhost:3000/wb/wb/wb/wb.html");
    ...
    const sectionParent = await page.$eval("#test-section", (el) => el.parentElement.id);
    (0, test_1.expect)(sectionParent).toBe("main-content");
});
//# sourceMappingURL=dynamicPagePositioning.spec.js.map