...
foreach ($jsFile in $fullJsFilePaths) {
    ...
    while ($runningJobs.Count -ge $maxConcurrentJobs) {
        ...
        foreach ($jobId in $runningJobs.Keys) {
            ...
            if ($job.Job.State -ne "Running") {
                ...
                if ($result -eq $true) {
                    $completedJobs++
                    Write-Host "Successfully converted: $jobId" -ForegroundColor Green
                } else {
                    $failedJobs++
                    Write-Host "Failed to convert: $jobId" -ForegroundColor Red
                }
    ...
    $jobScriptBlock = {
        ...
        function Fix-TypeScriptErrors {
            ...
            $fixedContent = $fixedContent -replace 'const\s+(\w+)\s*=\s*require\([''"](.+?)[''"]\)', 'import $1 from "$2"'
        ...
        function Test-TypeScriptCompilation {
            ...
            try {
                # Run tsc on the file
                $tscOutput = & npx tsc --noEmit $filePath 2>&1
        ...
        if (Test-Path $jsFilePath) {
            ...
            try {
                ...
                while (-not $compileSuccess -and $compilationTries -lt $maxTries) {
                    ...
                    # Test compilation
                    $result, $lastErrors = Test-TypeScriptCompilation -filePath $tsFilePath
                    $compileSuccess = $result
                }
                
                # If successful or max tries reached, remove the JS file
                if (Test-Path $tsFilePath) {
                    ...
                    if (Test-Path $jsFilePath) {
                        Write-Error "Failed to remove $jsFilePath despite no error"
                        return $false
                    }
                    ...
                    if ($compileSuccess) {
                        ...
                        Write-Error "File $tsFilePath converted but has compilation errors: $lastErrors"
                        return @{
                            Success = $true
                            Compiles = $false
                            Tries = $compilationTries
                ...
                Write-Error "Error processing $jsFilePath : $_"
            ...
            Write-Error "File not found: $jsFilePath"
        }
    ...
    foreach ($jobId in $runningJobs.Keys) {
        ...
        if ($job.Job.State -ne "Running") {
            ...
            if ($result -eq $true) {
                ...
                # Double-check that JS file is removed
                if (Test-Path $jobId) {
                    Write-Host "Removing leftover file: $jobId" -ForegroundColor Yellow
                    Remove-Item -Path $jobId -Force -ErrorAction SilentlyContinue
                }
            } else {
                $failedJobs++
                Write-Host "Failed to convert: $jobId" -ForegroundColor Red
