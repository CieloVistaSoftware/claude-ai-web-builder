...
function scanComponent(componentsPath, componentName) {
...
connection.onDefinition((params) => {
    ...
    if (componentName && componentIndex.has(componentName)) {
        ...
        return {
            ...
            range: node_1.Range.create(node_1.Position.create(symbol.definitionLine, 0), node_1.Position.create(symbol.definitionLine, 100))
        };
    }
    return null;
});
...
connection.onHover((params) => {
    ...
    if (componentName && componentIndex.has(componentName)) {
        ...
        if (symbol.cssFile)
            contents += `CSS: ${symbol.cssFile}\n`;
        if (symbol.demoFile)
            contents += `Demo: ${symbol.demoFile}\n`;
        if (symbol.mdFile)
            contents += `Docs: ${symbol.mdFile}\n`;
        return {
            contents: {
                kind: 'markdown',
                value: contents
...
function getComponentNameAtPosition(text, offset) {
            ...
            try {
                ...
                lines.forEach((line, lineNum) => {
                    ...
                    while ((match = regex.exec(line)) !== null) {
                        usages.push({
                            uri: `file://${filePath}`,
                            range: node_1.Range.create(node_1.Position.create(lineNum, match.index), node_1.Position.create(lineNum, match.index + componentName.length + 1))
                        });
                    }
                });
            }
