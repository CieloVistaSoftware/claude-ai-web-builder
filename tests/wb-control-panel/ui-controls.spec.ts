...
async function gotoWithRetry(page: Page, url: string, options = {}) {
  ...
  while (retries < maxRetries) {
    try {
      ...
      console.log(`Navigation failed (${retries + 1}/${maxRetries}):`, errorMessage);
      retries++;
...
test.describe('Website Builder UI Controls', () => {
  const baseTest = new BaseUnitTest();
  test('Basic UI controls are present and working', async ({ page }) => {
    await baseTest.createStandardTest(page, async () => {
      try {
          ...
          const hasStatusUpdate = await page.evaluate(() => {
            ...
            if (typeof (window as any).updateStatus === 'function') {
              (window as any).updateStatus('Test message', 'info');
  ...
  test('Import and file stacking buttons are working', async ({ page }) => {
    await baseTest.createStandardTest(page, async () => {
      try {
          await gotoWithRetry(page, 'http://localhost:3000/index.html');
