...
test.describe('Table Implementation Tests', () => {
  test.setTimeout(30000);

  test('should have valid table.json with required datasets', async ({ page }) => {
    ...
    expect(jsonContent.employees, 'Should have employees dataset').toBeDefined();
    expect(jsonContent.products, 'Should have products dataset').toBeDefined();
    expect(jsonContent.transactions, 'Should have transactions dataset').toBeDefined();

    // Verify datasets have records
    expect(jsonContent.employees.length, 'Employees dataset should have records').toBeGreaterThan(0);
    expect(jsonContent.products.length, 'Products dataset should have records').toBeGreaterThan(0);
    expect(jsonContent.transactions.length, 'Transactions dataset should have records').toBeGreaterThan(0);
  ...
  test('should load table.html with proper API usage', async ({ page }) => {
    ...
    expect(apiUsage.usesSetColumns, 'Should use setColumns method').toBe(true);
    expect(apiUsage.usesSetData, 'Should use setData method').toBe(true);
    expect(apiUsage.avoidsLoadData, 'Should not use deprecated loadData method').toBe(true);
  });

  test('should display data from table.json correctly', async ({ page }) => {
    await page.goto('/components/table/table.html');
    await page.waitForLoadState('networkidle');
    ...
    expect(headerCount, 'Table should have header columns').toBeGreaterThan(0);
    ...
    if (rowCount > 0) {
      ...
      expect(firstCellText, 'First cell should have content').toBeTruthy();
    }
  });

  test('should handle dataset switching', async ({ page }) => {
    await page.goto('/components/table/table.html');
    await page.waitForLoadState('networkidle');
    ...
    if (await datasetSelector.count() > 0) {
      ...
      if (options.length > 1) {
        ...
        expect(newRows, 'Table should have rows after dataset switch').toBeGreaterThan(0);
      }
    }
  });

  test('should have proper column mappings for each dataset', async ({ page }) => {
    ...
    expect(meaningfulHeaders.length, 'Should have meaningful column headers').toBeGreaterThan(0);
    ...
    if (jsonContent.employees && jsonContent.employees.length > 0) {
      ...
      const matchingHeaders = headerTexts.filter(header => 
        employeeFields.some(field => 
          header && header.toLowerCase().includes(field.toLowerCase())
        )
      );
      
      // This is a flexible check - we expect some correlation
      expect(matchingHeaders.length, 'Headers should relate to data fields').toBeGreaterThanOrEqual(0);
    }
  });
  ...
  test('should handle empty datasets gracefully', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      if (table && (window as any).tableComponent) {
        try {
          (window as any).tableComponent.setData([]);
        } catch (e) {
    ...
    expect(criticalErrors.length, `Should handle empty data without critical errors: ${criticalErrors.join(', ')}`).toBe(0);
  });

  test('should support table sorting if implemented', async ({ page }) => {
    await page.goto('/components/table/table.html');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1000);
    ...
    if (await sortableHeaders.count() > 0) {
      ...
      expect(firstRowAfterSort, 'Should still have data after sort attempt').toBeTruthy();
    }
  });
