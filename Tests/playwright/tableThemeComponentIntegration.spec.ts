...
test.describe("Table Theme Component Integration Tests", () => {
  test.setTimeout(30000);

  test("should have proper table-theme.html structure", async ({ page }) => {
    ...
    await expect(tableThemeComponent).toBeVisible();
    ...
    expect(colorBarScript.hasColorBarEvent).toBe(true);
    expect(colorBarScript.hasColorBarState).toBe(true);
    expect(colorBarScript.hasThemeControls).toBe(true);
  });

  test("should load table-theme-component.js with proper integration", async ({
    page,
  }) => {
    await page.goto("/components/table/table-theme.html");
    await page.waitForLoadState("networkidle");
    ...
    const componentIntegration = await page.evaluate(() => {
      ...
      const componentDefined =
        typeof customElements.get("table-theme-component") !== "undefined";
      ...
      return {
        ...
        hasCreateThemeFromColorBar:
          scriptContent.includes("createThemeFromColorBarState") ||
          scriptContent.includes("createTheme"),
        hasInitializeFromColorBar:
          scriptContent.includes("initializeFromColorBarState") ||
          scriptContent.includes("initialize"),
      };
    });

    expect(componentIntegration.componentDefined).toBe(true);
    expect(componentIntegration.hasColorBarListener).toBe(true);
  });

  test("should have all required component files accessible", async ({
    page,
  }) => {
    const requiredFiles = [
  ...
  test("should handle color bar state changes", async ({ page }) => {
    await page.goto("/components/table/table-theme.html");
    await page.waitForLoadState("networkidle");
  ...
  test("should create proper table component with theme integration", async ({
    page,
  }) => {
    await page.goto("/components/table/table-theme.html");
    await page.waitForLoadState("networkidle");
  ...
  test("should handle theme switching between light and dark modes", async ({
    page,
  }) => {
    await page.goto("/components/table/table-theme.html");
    await page.waitForLoadState("networkidle");
    ...
    if ((await darkModeToggle.count()) > 0) {
      ...
      expect(tableThemeUpdated).toBe(true);
    }
  });

  test("should integrate with main website builder color bar", async ({
    page,
  }) => {
    ...
    if ((await colorBar.count()) > 0) {
      ...
      await expect(page.locator("table-theme-component")).toBeVisible();
    }
  });
});
