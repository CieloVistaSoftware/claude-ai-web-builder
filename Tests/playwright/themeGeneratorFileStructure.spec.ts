...
test.describe('Theme Generator File Structure Tests', () => {
  test.setTimeout(30000);

  test('should have proper theme directory structure', async ({ page }) => {
    // Test main themes directory accessibility
    const themesResponse = await page.request.get('/themes/');
    expect([200, 404, 403]).toContain(themesResponse.status());
    ...
    expect(foundThemeFiles, 'Should have at least one theme file').toBeGreaterThan(0);
  });

  test('should have generator subdirectory if refactored', async ({ page }) => {
  ...
  test('should have proper file organization without legacy files', async ({ page }) => {
    ...
    for (const pattern of legacyPatterns) {
      ...
      expect(response.status(), `${pattern} should not exist`).toBe(404);
    }
  });

  test('should load theme generator with proper functionality', async ({ page }) => {
    ...
    if (response.status() === 200) {
      ...
      expect(elementCount, 'Should have theme generator elements').toBeGreaterThan(0);
    }
  });

  test('should have CSS and JavaScript files properly organized', async ({ page }) => {
    ...
    expect(foundCssFile || foundJsFile, 'Should have organized theme files').toBe(true);
  });

  test('should not have duplicate theme files', async ({ page }) => {
    ...
    for (const [file1, file2] of potentialDuplicates) {
      ...
      if (response1.status() === 200 && response2.status() === 200) {
        ...
        expect(isRedirect, `${file1} and ${file2} should not be identical duplicates`).toBe(true);
      }
    }
  });
  ...
  test('should have proper meta tags and SEO structure', async ({ page }) => {
    ...
    for (const themePage of themePages) {
      ...
      if (response.status() === 200) {
        ...
        expect(pageStructure.hasTitle, 'Should have page title').toBe(true);
        expect(pageStructure.hasDoctype, 'Should have DOCTYPE').toBe(true);
        expect(pageStructure.hasMetaViewport, 'Should have viewport meta tag').toBe(true);
  ...
  test('should have consistent file naming conventions', async ({ page }) => {
    ...
    for (const { pattern, description } of themeFilePatterns) {
      ...
      if (response.status() === 200) {
        ...
        if (cssResponse.status() === 200) {
          ...
          expect(contentType, `${cssFile} should have correct MIME type`).toContain('text/css');
        }
        
        if (jsResponse.status() === 200) {
          const contentType = jsResponse.headers()['content-type'];
          expect(contentType, `${jsFile} should have correct MIME type`).toMatch(/application\/javascript|text\/javascript/);
        }
      }
    }
  });
