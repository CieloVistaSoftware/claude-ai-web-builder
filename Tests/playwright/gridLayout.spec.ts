...
test.describe("Grid Layout Tests", () => {
  test("should calculate correct grid layout proportions using golden ratio", async () => {
    const goldenRatio = 1.618;
    const navWidthPercent = Math.round(100 / (goldenRatio * 2.75));
    const contentWidthPercent = 100 - navWidthPercent;

    // Verify calculations
    expect(navWidthPercent).toBeCloseTo(22, 0); // Should be approximately 22%
    expect(contentWidthPercent).toBeCloseTo(78, 0); // Should be approximately 78%

    console.log(`Navigation width: ${navWidthPercent}%`);
    console.log(`Content width: ${contentWidthPercent}%`);
  });

  test("should have correct responsive breakpoints", async () => {
  ...
  test("should apply grid layout in browser", async ({ page }) => {
    ...
    if ((await gridContainer.count()) > 0) {
      ...
      expect(gridDisplay).toMatch(/^(grid|flex|block)$/);
    }
  });

  test("should have proper grid template areas for different layouts", async ({
    page,
  }) => {
    await page.goto("/wb/wb/wb/wb.html");
