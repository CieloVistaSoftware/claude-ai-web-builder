...
test.describe("Navigation Width Tests", () => {
  test("should calculate navigation width using golden ratio", async () => {
    ...
    // Verify calculations
    expect(navWidthPercent).toBeCloseTo(22.4, 1);
    expect(goldenRatio).toBeCloseTo(1.618, 3);
    expect(inverseGoldenRatio).toBeCloseTo(0.618, 3);

    console.log(`Golden Ratio: ${goldenRatio}`);
    console.log(`Nav width percentage: ${navWidthPercent.toFixed(2)}%`);
  });

  test("should calculate responsive nav widths for different viewports", async () => {
    const goldenRatio = 1.618;
    ...
    viewports.forEach((viewport) => {
      const navWidthPixels = Math.round(
        viewport.width * (navWidthPercent / 100)
      );
      console.log(
        `${viewport.name} (${viewport.width}px): Nav width = ${navWidthPixels}px`
      );

      // Verify the calculation makes sense
      expect(navWidthPixels).toBeGreaterThan(0);
      expect(navWidthPixels).toBeLessThan(viewport.width);
    });
  });

  test("should apply navigation width in browser", async ({ page }) => {
    ...
    if (navElements.length > 0) {
      ...
      console.log(
        `Navigation width: ${navWidth.pixels}px (${navWidth.percentage.toFixed(
          1
        )}%)`
      );
      ...
      expect(navWidth.percentage).toBeGreaterThan(10);
      expect(navWidth.percentage).toBeLessThan(40);
    }
  });

  test("should maintain golden ratio proportions at different screen sizes", async ({
    page,
  }) => {
    const goldenRatio = 1.618;
    ...
    for (const size of viewportSizes) {
      ...
      if (navElements.length > 0) {
        ...
        console.log(
          `${size.width}x${size.height}: Nav = ${navData.widthPercent.toFixed(
            1
          )}%`
        );

        // Allow some tolerance for different implementations
        expect(navData.widthPercent).toBeGreaterThan(15);
        expect(navData.widthPercent).toBeLessThan(35);
      }
    }
  });
});
