...
class WebSocketDependencyLoader {
    constructor(socket, options = {}) {
        this.cacheTTL = 60 * 60 * 1000; // 1 hour cache TTL by default
        this.resourceTimeout = 10000; // 10 seconds timeout for resource loading
        ...
        if (options.resourceTimeout)
            this.resourceTimeout = options.resourceTimeout;
     ...
     */
    async loadPlugin(name, execute = true) {
        try {
     ...
     * @private
     */
    _fetchResource(resourceType, path) {
     ...
     */
    getLoadedContent(path) {
        return this.loaded.get(path) || null;
    }
}
// Create global instance if socket is available
if (typeof window !== 'undefined' && window.socket && !window.wsLoader) {
    window.wsLoader = new WebSocketDependencyLoader();
