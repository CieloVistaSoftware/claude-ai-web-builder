...
(function() {
    ...
    // Color change listener from control panel (PRIMARY FIX)
    document.addEventListener('wb:color-changed', (e) => {
        const { hue, saturation, lightness, harmonyMode } = e.detail;
        
        if (typeof hue !== 'undefined' && typeof saturation !== 'undefined' && typeof lightness !== 'undefined') {
            logEvent(`Applying color change - H:${hue} S:${saturation}% L:${lightness}% Mode:${harmonyMode}`);
            
            const root = document.documentElement;
            
            // Set PRIMARY color foundation (HSL values)
            ...
            // Set PRIMARY color (actual color)
            ...
            logEvent(`✅ PRIMARY COLOR APPLIED: ${primaryColor}`);
        }
    });
