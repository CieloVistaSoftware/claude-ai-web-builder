...
class WebsiteBuilderWizard extends HTMLElement {
  ...
  private renderWebsiteTypeStep(): string {
    return `
  ...
  private setupEventListeners() {
    ...
    const websiteTypeSelect = this.shadowRoot!.getElementById('website-type') as HTMLSelectElement;
    if (websiteTypeSelect) {
      websiteTypeSelect.addEventListener('change', () => {
        ...
        if (descriptionElement) {
          ...
          descriptionElement.innerHTML = descriptions[selectedType as keyof typeof descriptions] || '';
        }
      });
    }
  }
